...
public class writeRaster {
    ...
    /**
     * Turbo colormap lookup table (256 RGB triplets, values 0-1)
     */
    private static final double[][] TURBO_COLORMAP = {
    ...
    /**
     * Convert a normalized value (0-1) to a turbo colormap RGB value using lookup table with interpolation
     * @param value normalized value between 0 and 1
     * @return ARGB color value
     */
    private int getTurboColor(double value) {
        // Clamp value between 0 and 1
        value = Math.max(0, Math.min(1, value));

        // Map to lookup table index
        double index = value * (TURBO_COLORMAP.length - 1);
        int lowerIndex = (int) index;
        int upperIndex = Math.min(lowerIndex + 1, TURBO_COLORMAP.length - 1);
        double fraction = index - lowerIndex;

        // Linear interpolation between adjacent colors
        double[] lowerColor = TURBO_COLORMAP[lowerIndex];
        double[] upperColor = TURBO_COLORMAP[upperIndex];

        double r = lowerColor[0] + fraction * (upperColor[0] - lowerColor[0]);
        double g = lowerColor[1] + fraction * (upperColor[1] - lowerColor[1]);
        double b = lowerColor[2] + fraction * (upperColor[2] - lowerColor[2]);
    ...
    public void writeToRaster(double[][] constructionGrid) throws IOException {
        ...
        for (int i = 0; i < constructionGrid.length; i++) {
            for (int j = 0; j < constructionGrid[0].length; j++) {
                double val = constructionGrid[i][j];
                if (val != 0) {
